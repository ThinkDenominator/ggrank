...
A value, mark, score, or rate is not required when authoritative ranks already
exist. Supply `category`, `period`, and `rank`:
...
The chart uses a compact category-and-rank layout and does not draw an empty
value column. Ranks must be positive whole numbers. If several institutions
have separate rankings in the same period, analyse each institution separately
